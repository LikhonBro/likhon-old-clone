...
{color.Y}   ✨🌟✨ {color.W}『{color.Y}L I K H O N{color.W}』{color.Y} ✨🌟✨
{color.Y}   🦁💛 𝙋𝙍𝙀𝙈𝙄𝙐𝙈 𝙏𝙊𝙊𝙇 𝘽𝙔 𝙇𝙄𝙆𝙃𝙊𝙉 💛🦁
{color.P}╔═══════════════════════════════════════════════════════╗
{color.P}║{color.C}                ✦  𝗧𝗢𝗢𝗟 𝗜𝗡𝗙𝗢 𝗣𝗔𝗡𝗘𝗟  ✦                  {color.P}║
{color.P}╚═══════════════════════════════════════════════════════╝
{color.Y}   ➤ {color.W}Creator        : {color.Y}LIKHON 🦁
{color.Y}   ➤ {color.W}Tool Access    : {color.G}PAID 💸
{color.Y}   ➤ {color.W}Current Version: {color.P}0.2
{color.G}─────────────────────────────────────────────────────────{color.N}\n''')
...
class LIKHONCracker:
    ...
    def old_menu(self):
        ...
        print(f'{self.color.P}║ {self.color.C}[3] {self.color.Y}Contact Creator                     {self.color.P}║')
        ...
        if choice in ('1', '01'):
            ...
            os.system('xdg-open https://t.me/Mrlikhon9')
            print(f'\n  {self.color.Y}Opening Telegram...')
            time.sleep(2)
            self.old_menu()
        elif choice in ('0', '00'):
            return
    ...
    def breach_target(self, target, passlist):
        self.loop += 1
        sys.stdout.write(f'\r  {self.color.W}[LIKHON] {self.loop}|{self.color.R}{len(self.oks)}|{self.color.G}{len(self.cps)}{self.color.W}')
        sys.stdout.flush()
    ...
    def handle_success(self, uid, password, response):
        ...
        with open('/sdcard/LIKHON-OLD.txt', 'a') as f:
